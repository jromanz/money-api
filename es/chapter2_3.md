...
Para crear una instancia de ```MonetaryAmount```, todas las implementaciones siguen el mismo standard de nomenclatura utilizando, con una pequeña excepción en ```RoundedMoney``` una vez que el pueda recibir un ```MonetaryOperator``` para trabajar como “redondeador” en cada operación. Listando los mas importantes tenemos el total de tres métodos, que son:
